...
// v3.16.2-netproxy · 有界复用池 · 防 globalAgent 无限新建 socket 打满 conntrack
...
// v3.16.2 · 多实例隔离 · 道生一一生二 · 邻邦相望·鸡狗之声相闻·民至老死不相往来
//   WAM_HOME 环境变量 → 不同 --user-data-dir 实例 → 不同 ~/.wam-<name> → 完全独立
let WAM_DIR = process.env.WAM_HOME || path.join(os.homedir(), ".wam");
const WAM_INSTANCE = process.env.WAM_INSTANCE || null; // v3.16.2 · 实例名标识
let STATE_FILE = path.join(WAM_DIR, "wam-state.json");
...
let LOCK_FILE = path.join(WAM_DIR, "lock-state.json");
// v3.7.6 · dismiss 跨窗口持久化 · 小邦寺民·各得其欲·大者宜为下
let DISMISS_FILE = path.join(WAM_DIR, "_conv_dismiss.json"); // uuid→ts 共享持久文件·多窗口共守
const CONV_DISMISS_TTL = 600000; // 10min dismiss 自动过期时长
// v13.2 · 通知一次性闸门 · 跨窗口共享 · 同一异常生命周期只弹一次
let CONV_NOTIFY_DIR = path.join(WAM_DIR, "_conv_notify_claims"); // uuid.json 存在 = 本轮异常已提示
let BACKUP_DIR = path.join(WAM_DIR, "backups");
let PENDING_TOKEN_FILE = path.join(WAM_DIR, "_pending_token.json");
const MAX_BACKUPS = 10;
// v3.4.0 · 三界归一 · Hub 总线 + 对话追踪 (卅辐同一毂·当其无有·车之用也)
let HUB_FILE = path.join(WAM_DIR, "_hub.json");
// v3.16.2 · 多实例 cascade 隔离 · CODEIUM_DATA_DIR 环境变量
let CODEIUM_BASE = process.env.CODEIUM_DATA_DIR || path.join(os.homedir(), ".codeium");
let PB_DIR =
  _resolveCascadePbDir() ||
  path.join(CODEIUM_BASE, "windsurf", "cascade"); // v3.16.0: 动态解析 Devin/Windsurf
let CONV_BACKUP_DEFAULT = path.join(WAM_DIR, "conversation_backups");
...
let L6_CLAIM_DIR = path.join(WAM_DIR, "_l6_claim");
// v3.16.2 · 多实例隔离 · 重新定位 WAM_DIR 及所有依赖路径
//   帛书·三十九: 昔之得一者 · 天得一以清 · 地得一以寧 · 侯王得一以為天下正
function _rebaseWamDir(newDir) {
  WAM_DIR = newDir;
  STATE_FILE = path.join(WAM_DIR, "wam-state.json");
  LOCK_FILE = path.join(WAM_DIR, "lock-state.json");
  DISMISS_FILE = path.join(WAM_DIR, "_conv_dismiss.json");
  CONV_NOTIFY_DIR = path.join(WAM_DIR, "_conv_notify_claims");
  BACKUP_DIR = path.join(WAM_DIR, "backups");
  PENDING_TOKEN_FILE = path.join(WAM_DIR, "_pending_token.json");
  HUB_FILE = path.join(WAM_DIR, "_hub.json");
  CONV_BACKUP_DEFAULT = path.join(WAM_DIR, "conversation_backups");
  L6_CLAIM_DIR = path.join(WAM_DIR, "_l6_claim");
  _KEY_CACHE = path.join(WAM_DIR, "_cascade_key.json");
  BACKUP_META_FILE = path.join(WAM_DIR, "_backup_meta.json");
  AGENT_API_FILE = path.join(WAM_DIR, "_api.json");
  SESSION_CACHE_FILE = path.join(WAM_DIR, "_session_cache.json");
  log("多实例: WAM_DIR 重定位 → " + WAM_DIR);
}

// ═══ v3.16.2+ · 多实例一键启动 · 道生一一生二 · 邻邦相望·鸡狗之声相闻·民至老死不相往来 ═══
// 帛书·四十二: 道生一，一生二，二生三，三生万物
// 帛书·八十: 邻邦相望，鸡狗之声相闻，民至老死不相往来
// 用户点按钮 → 自动创建目录 → 复制设置 → 复制扩展 → 启动新进程 → 完全隔离
// 无为而无不为 · 道法自然

function _discoverExePath() {
  // 优先: process.argv[0] — Electron 环境下指向实际可执行文件
  let exe = process.argv[0] || "";
  if (exe && exe.toLowerCase().includes("windsurf")) return exe;
  if (exe && exe.toLowerCase().includes("devin")) return exe;
  // 备选: process.execPath
  exe = process.execPath || "";
  if (exe && (exe.toLowerCase().includes("windsurf") || exe.toLowerCase().includes("devin"))) return exe;
  // 兜底: 常见安装路径
  const candidates = [
    ...
    path.join(process.env.PROGRAMFILES || "", "Devin", "Devin.exe"),
  ];
  for (const c of candidates) {
...
function _copyDirRecursive(src, dst, excludeExts) {
  if (!fs.existsSync(src)) return;
  ensureDir(dst);
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    const s = path.join(src, entry.name);
    const d = path.join(dst, entry.name);
    if (entry.isDirectory()) { _copyDirRecursive(s, d, excludeExts); }
    else {
      if (excludeExts && excludeExts.some(ext => entry.name.endsWith(ext))) continue;
      if (!fs.existsSync(d)) { // 不覆盖已有文件
...
async function _newInstance() {
  // ═══ v3.16.0+ · 多实例一键启动 · 道生一一生二 ═══
  //   反者道之动: 不弹输入框 · 直接开窗口 · 自动编号 · 无为而无不为
  //   邻邦相望·鸡狗之声相闻·民至老死不相往来 → 每个实例完全隔离

  // 1. 自动发现 IDE 可执行文件
  const exePath = _discoverExePath();
  if (!exePath) {
    vscode.window.showErrorMessage("无法发现 Devin/Windsurf 可执行文件 · 请确认已安装");
    return;
  }
  const isDevin = exePath.toLowerCase().includes("devin");
  const ideName = isDevin ? "Devin" : "Windsurf";
  const ideLower = ideName.toLowerCase();
  log("newInstance: exePath=" + exePath + " ideName=" + ideName);

  // 2. 自动编号: 扫描已有的 Devin-i* / Windsurf-i* 目录 → 取下一个编号
  const appdata = process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming");
  const home = os.homedir();
  let instNum = 1;
  try {
    const dirs = fs.readdirSync(appdata, { withFileTypes: true })
      .filter(d => d.isDirectory() && d.name.startsWith(ideName + "-i"))
      .map(d => {
        const m = d.name.match(/i(\d+)$/);
        return m ? parseInt(m[1]) : 0;
      })
      .filter(n => n > 0);
    if (dirs.length > 0) instNum = Math.max(...dirs) + 1;
  } catch {}
  const instName = "i" + instNum;
  log("newInstance: 自动编号 → " + instName);

  // 3. 计算所有隔离路径
  const dataDir = path.join(appdata, ideName + "-" + instName);
  const extDir = path.join(home, "." + ideLower + "-" + instName, "extensions");
  const wamHome = path.join(home, ".wam-" + instName);
  const codeiumDir = path.join(home, ".codeium-" + instName);

  // 4. 如果该实例目录已存在且 settings.json 已有配置 → 直接启动 (复用)
  const dstSettings = path.join(dataDir, "User", "settings.json");
  const alreadySetup = fs.existsSync(dstSettings) && fs.existsSync(extDir);

  if (!alreadySetup) {
    // 5. 创建目录结构 + 从默认实例复制完整 User 数据
    //   道法自然: Devin/Windsurf 需要完整的 User 目录才能启动
    //   但 state.vscdb 包含默认实例的 session → 新实例无法使用 → 崩溃
    //   方案: 先复制全部 → 再删除所有 vscdb → Devin 启动时自动重建
    ensureDir(dataDir);
    ensureDir(extDir);
    ensureDir(wamHome);
    const defaultDataDir = path.join(appdata, ideName);
    const srcUserDir = path.join(defaultDataDir, "User");
    const dstUserDir = path.join(dataDir, "User");
    if (fs.existsSync(srcUserDir)) {
      _copyDirRecursive(srcUserDir, dstUserDir);
      // 删除所有 vscdb 文件 (SQLite 数据库 · 包含旧 session · Devin 会自动重建)
      try {
        const vscdbFiles = [];
        function _collectVscdb(dir) {
          if (!fs.existsSync(dir)) return;
          for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            const p = path.join(dir, entry.name);
            if (entry.isDirectory()) _collectVscdb(p);
            else if (entry.name.endsWith(".vscdb") || entry.name.endsWith(".vscdb-wal") || entry.name.endsWith(".vscdb-shm")) {
              vscdbFiles.push(p);
            }
          }
        }
        _collectVscdb(dstUserDir);
        for (const f of vscdbFiles) { try { fs.unlinkSync(f); } catch {} }
        log("newInstance: 删除 " + vscdbFiles.length + " 个 vscdb → 自动重建");
      } catch (e) { log("newInstance: vscdb 清理失败: " + (e.message || e)); }
      log("newInstance: User 目录已复制 → " + dstUserDir);
    } else {
      ensureDir(path.join(dataDir, "User"));
      ensureDir(path.join(dataDir, "User", "globalStorage"));
    }

    // 6. 注入多实例配置 (覆盖已复制的 settings.json)
    try {
      let settings = {};
      if (fs.existsSync(dstSettings)) {
        settings = JSON.parse(fs.readFileSync(dstSettings, "utf8"));
      }
      settings["wam.multiInstance"] = true;
      settings["wam.wamHome"] = wamHome;
      settings["wam.codeiumDataDir"] = codeiumDir;
      // API 端口隔离: 8937 + 实例编号 (简单可靠)
      const apiPort = 8937 + instNum;
      const apiUrl = "http://127.0.0.1:" + apiPort;
      settings["codeium.apiServerUrl"] = apiUrl;
      settings["codeium.inferenceApiServerUrl"] = apiUrl;
      settings["wam.apiPort"] = apiPort;
      fs.writeFileSync(dstSettings, JSON.stringify(settings, null, 2));
      log("newInstance: 设置已注入 API端口=" + apiPort);
    } catch (e) { log("newInstance: 设置失败: " + (e.message || e)); }

    // 7. 复制所有扩展 (道法自然 · 新窗口体验与原生一致)
    const defaultExtDir = path.join(home, "." + ideLower, "extensions");
    if (fs.existsSync(defaultExtDir)) {
      try {
        const exts = fs.readdirSync(defaultExtDir, { withFileTypes: true })
          .filter(d => d.isDirectory() && !d.name.startsWith("_") && !d.name.startsWith("."));
        let copied = 0;
        for (const ext of exts) {
          const srcExt = path.join(defaultExtDir, ext.name);
          const dstExt = path.join(extDir, ext.name);
          if (!fs.existsSync(dstExt)) { _copyDirRecursive(srcExt, dstExt); copied++; }
        }
        log("newInstance: 复制 " + copied + "/" + exts.length + " 扩展");
      } catch (e) { log("newInstance: 扩展复制失败: " + (e.message || e)); }
    }
  } else {
    log("newInstance: 实例 " + instName + " 已有配置 · 直接启动");
  }

  // 8. 启动新进程 · 道法自然: 用临时批处理文件确保环境变量传递到 Electron 子进程
  //   spawn/exec 的 env 在 Windows Electron 下可能丢失
  //   批处理文件: set ENV → start exe → 环境变量一定生效 · 窗口一定独立
  const { exec } = require("child_process");
  const os_tmpdir = require("os").tmpdir();
  const batPath = path.join(os_tmpdir, "wam-launch-" + instName + ".bat");
  const batContent = [
    "@echo off",
    "set WAM_HOME=" + wamHome,
    "set WAM_INSTANCE=" + instName,
    "set CODEIUM_DATA_DIR=" + codeiumDir,
    'start "" "' + exePath + '" --user-data-dir "' + dataDir + '" --extensions-dir "' + extDir + '"',
  ].join("\r\n");
  try { fs.writeFileSync(batPath, batContent, "ascii"); } catch {}
  log("newInstance: 批处理 → " + batPath);

  vscode.window.setStatusBarMessage("正在启动 " + ideName + " " + instName + " ...", 5000);

  exec('cmd /c "' + batPath + '"', { windowsHide: true, timeout: 10000 }, (err) => {
    if (err) {
      log("newInstance: 启动失败: " + (err.message || err));
      vscode.window.showErrorMessage("启动实例失败: " + (err.message || err));
    } else {
      log("newInstance: ✓ " + ideName + " " + instName + " 已启动");
      vscode.window.setStatusBarMessage("✓ " + ideName + " " + instName + " 已启动 · 可登录不同账号", 5000);
    }
  });
}
...
let _output = null,
  ...
  _lastNoAvailNotifyAt = 0, // v3.16.2-fix · 无可用账号通知冷却 · 防看门狗2s循环刷屏
...
let _KEY_CACHE = path.join(WAM_DIR, "_cascade_key.json");
...
//
let BACKUP_META_FILE = path.join(WAM_DIR, "_backup_meta.json");
...
let AGENT_API_FILE = path.join(WAM_DIR, "_api.json");
...
<button onclick="doSetBackupDir()" class="conv-btn" title="配置备份目录·已有备份自动迁移·每日自动全量备份·新对话实时增量同步">&#128193; 备份配置</button> <button onclick="doNewInstance()" class="conv-btn" title="一键新开隔离实例·不同账号·互不干扰·道生一一生二">&#10010; 新实例</button>
...
<div class="conv-actions">
<button onclick="doSetBackupDir()" class="conv-btn" title="选择备份目录·已有备份自动迁移·每日自动备份·新对话实时同步">&#128193; 备份配置</button> <button onclick="doNewInstance()" class="conv-btn" title="一键新开隔离实例·不同账号·互不干扰·道生一一生二">&#10010; 新实例</button>
${backupStatus}
</div>
<div class="conv-backup-path" title="${_esc(backupDir)}">备份: ${_esc(backupDir)}</div>
</div></div>`;
...
let SESSION_CACHE_FILE = path.join(WAM_DIR, "_session_cache.json");
...
// v3.16.3-wam · 设备级限制 (too many free user accounts) · 官方 device fingerprint 限制
//   触发: registerUserViaSession / devinLogin 返回 "too many free user accounts" / "permission_denied"
//   治法: 暂停自动切号 30min · 通知用户 · 避免持续触发官方限制 (账号全灭)
let _deviceLimitedUntil = 0; // ms timestamp · 设备级限制窗口结束时间
const DEVICE_LIMIT_COOLDOWN_MS = 30 * 60 * 1000; // 30min
function _isDeviceLimited() {
  return Date.now() < _deviceLimitedUntil;
}
function _triggerDeviceLimit(reason) {
  _deviceLimitedUntil = Date.now() + DEVICE_LIMIT_COOLDOWN_MS;
  log("⚠️ 设备级限制: " + reason + " · 暂停自动切号 " + (DEVICE_LIMIT_COOLDOWN_MS / 60000) + "min");
  try {
    _notify("warning", "WAM: 官方设备级限制 (" + reason + ") · 暂停自动切号 30min · 可手动切号");
  } catch {}
}
function _cacheSession(email, sessionToken, apiKey, apiServerUrl, auth1) {
  ...
  _sessionCache.set(email.toLowerCase(), {
    ...
    auth1: auth1 || "", // v3.16.3 · Devin bearer · 供 billing/status 权威校验
    cachedAt: Date.now(),
    maxAgeMs,
  });
...
function _startHardExhaustWatchdog() {
  ...
  _hardExhaustWatchdogTimer = setInterval(() => {
    try {
      ...
      // v3.16.2-fix · 全部账号已锁定时 · 看门狗不再触发 _tick
      //   根因: 全锁 → _tick → bestI=-1 → 通知 → 2s 后看门狗再触发 → 循环
      //   治法: 检测全锁 → 跳过 · 省 CPU · 省日志 · 省通知
      const _allLocked = _store.accounts.every((a) => a.skipAutoSwitch);
      if (_allLocked) return;
...
class Store {
  ...
  getStats() {
    ...
    return {
      ...
      drought: checkedCount >= 2 && totalW / checkedCount < 1,
...
async function devinLogin(email, password) {
  ...
  try {
    ...
    // v3.16.3-wam · 设备级限制识别 (permission_denied 类)
    if (/too many free user accounts|permission_denied/i.test(String(err))) {
      _triggerDeviceLimit("too many free user accounts");
    }
...
async function registerUserViaSession(sessionToken) {
  try {
    ...
    // v3.16.3-wam · 设备级限制识别 (permission_denied 类)
    const _regErr = (r.json && (r.json.code || r.json.message)) || "no_api_key";
    if (/too many free user accounts|permission_denied/i.test(String(_regErr))) {
      _triggerDeviceLimit("too many free user accounts");
    }
    return {
      ok: false,
...
async function _tryDevinBillingFallback(auth1) {
  ...
  try {
    ...
    // ── 额度真值 (实证 2026-07-01 · 144号活体全覆盖) ────────────────────────
    //   billing/status 为权威账单: overage_credits>0 且 billing_error=null 且
    //   has_subscription_or_credits!==false → 有真实额度; 反之(负值/含错/无订阅)=耗尽。
    //   ⚠ 原逻辑把 <0 当有钱(符号颠倒) → 有余额号反显 $0 · 已订正为正=有钱。
    const _exhausted =
      !!j.billing_error ||
      j.has_subscription_or_credits === false ||
      typeof j.overage_credits !== "number" ||
      j.overage_credits <= 0;
    const hasFunds = !_exhausted;
    const dollarAmt = hasFunds ? Math.round(j.overage_credits * 100) / 100 : 0;
    return {
      checked: true,
...
// v3.16.3 · 额度真值权威校验 (实证 2026-07-01 · 144号活体) ────────────────────
//   GetUserStatus.overageBalanceMicros 会滞留「已授予正值」(如耗尽 Free 号缓存仍报 $68),
//   而 billing/status 才是权威账单。∴ 凡 GetUserStatus 报正额, 必经此校验:
//     耗尽(billing_error / 无订阅 / overage_credits<=0) → 归零 $0 (对齐手机版 & 用户实证)
//     有额 → 采用账单 overage_credits 精确两位美金 (与 micros 一致但更准)
async function _devinBillingTruth(auth1) {
  if (!auth1) return null;
  try {
    const orgId = await _getOrgId(auth1);
    if (!orgId) return null;
    ...
    const oc = typeof j.overage_credits === "number" ? j.overage_credits : null;
    const exhausted =
      !!j.billing_error ||
      j.has_subscription_or_credits === false ||
      oc == null ||
    ...
    return {
      exhausted,
      credits: exhausted ? 0 : Math.round(oc * 100) / 100,
      billingError: j.billing_error || null,
    };
  } catch {
    return null;
...
// 正额校验器: q.overageDollars>0 时才发账单请求 (已 $0 无需校验·省调用)
async function _reconcileOverage(q, auth1) {
  if (!q || !auth1) return q;
  if (!(q.overageDollars > 0)) return q;
  try {
    const t = await _devinBillingTruth(auth1);
    if (!t) return q; // 账单不可达 → 保留 GetUserStatus 值 (best-effort)
    if (t.exhausted) {
      q.overageDollars = 0;
      q.overageActive = false;
    } else if (t.credits > 0) {
      q.overageDollars = t.credits;
      q.overageActive = true;
    }
  } catch {}
  return q;
}
...
async function verifyOneAccount(account) {
  // v3.16.3-wam · 设备级限制入口检查 (零网络 · 立返)
  if (_isDeviceLimited()) {
    return { ok: false, stage: "device-limited", error: "device-limited" };
  }
  ...
  if (_cachedV) {
    ...
    if (_qC) {
      await _reconcileOverage(_qC, _cachedV.auth1); // 正额必经账单权威校验·耗尽归零
      _cacheSession(
        account.email,
  ...
  await _reconcileOverage(q, dl.auth1); // 正额必经账单权威校验·耗尽归零(实证2026-07-01)
  // v3.0.3 · 🚀 验证阶段缓存 sessionToken · 下次切号可跳 devinLogin (道法自然·预赋)
  _cacheSession(account.email, pa.sessionToken, apiKey, apiServerUrl, dl.auth1);
...
async function loginAccount(store, idx) {
  // v3.16.3-wam · 设备级限制入口检查 (零网络 · 立返)
  if (_isDeviceLimited()) {
    const remainMin = Math.ceil((_deviceLimitedUntil - Date.now()) / 60000);
    log("login: 设备级限制中 · 剩余 " + remainMin + "min · 跳过");
    return { ok: false, error: "device-limited", stage: "preCheck" };
  }
  ...
  _cacheSession(acc.email, pa.sessionToken, _regApiKey, _regApiServerUrl, dl.auth1);
...
function _broadcastUI() {
  ...
  _broadcastUITimer = setTimeout(() => {
    ...
    // v3.16.2-fix · 添加账号面板展开时 · 跳过全量 HTML 重建 · 保住 textarea 焦点/光标
    //   根因: _editorPanel.webview.html = buildHtml() 全量替换 DOM → textarea 被销毁重建
...
function _resolveCascadePbDir() {
  // v3.16.2 · 多实例 cascade 隔离 · CODEIUM_DATA_DIR 优先
  //   帛书·八十: 邻邦相望，鸡狗之声相闻，民至老死不相往来
  //   不同实例的 cascade 目录完全独立 · 互不干扰
  const home = os.homedir();
  // v3.16.2 · CODEIUM_DATA_DIR 环境变量 → 实例专属 codeium 目录
  if (process.env.CODEIUM_DATA_DIR) {
    const cdir = process.env.CODEIUM_DATA_DIR;
    const instCandidates = [
      path.join(cdir, "windsurf", "cascade"),
      path.join(cdir, "devin", "cascade"),
      path.join(cdir, "Devin", "cascade"),
    ];
    for (const p of instCandidates) {
...
function buildHtml() {
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    <div class="row${isActive ? " act" : ""}${isBanned ? " banned" : ""}${isInUse ? " inuse" : ""}${!claudeOk && h.checked ? " expired-row" : ""}${isStaleRow ? " is-stale" : ""}" data-i="${i}" data-email="${_esc(a.email.toLowerCase())}">
      ...
      ${expTag}${planTag}${h.checked && h.overageDollars > 0 ? (h.staleHours >= 6 ? `<span class="eua-stale" title="Extra Usage $${h.overageDollars.toFixed(0)} · 数据${h.staleHours}h前(可能已消耗·建议重验)">$${h.overageDollars.toFixed(2)}?</span>` : `<span class="eua" title="Extra Usage Active · $${h.overageDollars.toFixed(0)} · Cascade quota=0时仍完全可用${h.staleHours >= 1 ? " · " + h.staleHours + "h前验" : ""}">$${h.overageDollars.toFixed(2)}</span>`) : ""}${h.checked && !h.overageDollars ? `<span class="eua0" title="已验 · 无Extra Usage余额">$0</span>` : ""} ${claudeTag}${bnTag}${iuTag}${staleTag}${freshTag}${liveTag}${ucTag}
      <span class="qt">
...
function doNewInstance(){vscode.postMessage({type:'newInstance'});}
...
async function handleWebviewMessage(msg) {
  try {
    switch (msg.type) {
      ...
      case "setAddOpen": {
        _uiAddOpen = !!msg.open;
        // v3.16.2-fix · 面板关闭时立即触发一次全量刷新 · 补回期间被抑制的 UI 更新
        if (!_uiAddOpen) _broadcastUI();
        break;
      }
      // v3.4.0 · 对话备份 · 道法自然
...
class Engine {
  ...
  async rotateNext(opts) {
    ...
    try {
      ...
      const RE_RATE_LIMIT = /rate.?limit|too.?many.?request|429|too many free user accounts|permission_denied/i;
      ...
      for (const idx of order) {
        ...
        if (r.ok) return r;
        // v3.16.3-wam · 设备级限制 → 30min 冷却 (比 rate-limit 更严重 · 全号皆败)
        if (r.error && /too many free user accounts|permission_denied/i.test(String(r.error))) {
          _triggerDeviceLimit("too many free user accounts");
          rateLimitHit = true;
          break;
        }
  ...
  async _tick() {
    ...
    // v3.16.3-wam · 设备级限制 → 暂停自动切号 (手动切号不受限)
    if (_isDeviceLimited()) return;
    ...
    if (isHardExhausted && !_switching) {
      ...
      if (bestI >= 0) {
        ...
        if (_allLocked) {
          // 全部锁定 → 用户主动选择 · 不越权不通知
          if (_nowHe - _lastNoAvailNotifyAt > 60000) {
            log("硬耗尽: " + reason + ", 全部账号已锁定 · 尊重用户选择 · 不越权");
...
async function activate(context) {
  ...
  // v3.16.2 · 多实例隔离 · 环境变量诊断日志
  //   太上下知有之 · 民莫之令而自均焉
  if (WAM_INSTANCE) log("多实例: WAM_INSTANCE=" + WAM_INSTANCE);
  if (process.env.WAM_HOME) log("多实例: WAM_HOME=" + process.env.WAM_HOME);
  if (process.env.CODEIUM_DATA_DIR) log("多实例: CODEIUM_DATA_DIR=" + process.env.CODEIUM_DATA_DIR);
  if (process.env.WAM_MODE_DEFAULT) {
    _wamMode = process.env.WAM_MODE_DEFAULT;
    log("多实例: WAM_MODE_DEFAULT=" + _wamMode);
  }
  // v3.16.2 · 配置覆盖 (环境变量未设置时，读 wam.wamHome / wam.codeiumDataDir)
  //   帛书·十七: 功成事遂 · 百姓皆谓我自然
  try {
    const cfgWamHome = _cfg("wamHome", "");
    if (cfgWamHome && !process.env.WAM_HOME) {
      _rebaseWamDir(cfgWamHome);
      CODEIUM_BASE = path.join(path.dirname(cfgWamHome), ".codeium");
      PB_DIR = _resolveCascadePbDir() || path.join(CODEIUM_BASE, "windsurf", "cascade");
    }
    const cfgCodeiumDir = _cfg("codeiumDataDir", "");
    if (cfgCodeiumDir && !process.env.CODEIUM_DATA_DIR) {
      CODEIUM_BASE = cfgCodeiumDir;
      PB_DIR = _resolveCascadePbDir() || path.join(CODEIUM_BASE, "windsurf", "cascade");
    }
  } catch (e) { log("多实例配置覆盖: " + (e.message || e)); }
...
function deactivate() {
  ...
  // v3.16.3-wam · 强制落盘 sessionCache (debounce 100ms 定时器可能被进程退出清除 → 重启丢 token)
  try {
    if (_persistDebounceTimer) clearTimeout(_persistDebounceTimer);
    const _obj = {};
    const _now = Date.now();
    for (const [email, c] of _sessionCache.entries()) {
      if (_now - c.cachedAt < SESSION_CACHE_DISK_TTL_MS) {
        _obj[email] = {
          sessionToken: c.sessionToken,
          apiKey: c.apiKey || "",
          apiServerUrl: c.apiServerUrl || "",
    ...
    atomicWrite(SESSION_CACHE_FILE, JSON.stringify(_obj, null, 0));
    log("deactivate: sessionCache 强制落盘 " + Object.keys(_obj).length + " 个");
  } catch (e) {
    log("deactivate persist fail: " + (e.message || e));
  }
  log("WAM deactivate");
}
...
module.exports = {
  ...
  _internals: {
    ...
    // v3.16.3-wam · 设备级限制状态
    get _deviceLimitedUntil() {
      return _deviceLimitedUntil;
    },
    _triggerDeviceLimit(t) {
      _triggerDeviceLimit(t || "manual");
    },
