...
let _output = null,
  ...
  _hubLastStuckUuids = new Map(), // uuid → { ts, name, count } (带时间过期 · 5分钟自动清理)
...
// v13.0 · 知止不殆·可以长久 — 通知最小化哲学:
//   「一」每个卡住对话只提醒一次 (不重复) — 用户知悉即可·无需反复打扰
//   「二」10分钟无操作自动消失 (不需用户手动关闭) — 提醒有时效·过期则静默
const STUCK_STALE_MAX = 600; // 秒: staleSec 超过此值 → 不显示 (备用安全网)
const STUCK_NOTIFY_MAX_PER_UUID = 1; // v13.0: 每个卡住对话只提醒一次 (知止不殆)
