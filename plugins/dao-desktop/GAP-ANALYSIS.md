...
| 会话 rename/archive/delete 跨侧变更矩阵 | ✅ | /api/cascade/{rename,archive,delete} 写官方真源已在位; R158 新增 `/api/cascade/matrix-roundtrip`(sync-rpc.sessionMatrixRoundtrip): rename/archive 写→经**另一读路径** GetAllCascadeTrajectories 复读→原样还原, 后端可验跨侧同源(delete 破坏性不入探针); 并入 /api/sync/rpc-roundtrip |
