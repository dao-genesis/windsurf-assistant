...
- **R122 官方二级页缺口补齐（ACP 连接控制 + 全局 Workflow/Skill 创建）**：①设置板块新增「ACP 连接」区——本地注册表 registry.json 打开/初始化(官方 devin.openAcpLocalRegistry 同源：不存在则建 `{version:"1.0.0",agents:[]}` 以 jsonc 打开)、重载 ACP 连接(停掉 Cascade 面板持有的 ACP 客户端并清退避，下次消息懒重连；宿主有官方 devin.reloadAcpConnections 命令时一并触发)、Devin Local 连接活体状态。extension.js 把 cascade provider 经 opts 注入归一面板。②Cascade 面板 Workflows/Skills 区新增 ⊕ 全局创建入口：CreateCustomizationFile GLOBAL_WORKFLOWS/GLOBAL_SKILLS(官方 processCreateWorkflow 同源，全局变体不传 workspaceConfigDir，落 ~/.codeium/windsurf 下)
