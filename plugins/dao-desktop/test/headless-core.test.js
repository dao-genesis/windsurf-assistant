...
// R214 · 空态/计数/发送钮官方同文批量。
test("R214: No results / N of M / Send / Cancel step 官方同文", () => {
  const src = fs.readFileSync(path.join(ROOT, "dao-cascade", "panel.js"), "utf8");
  assert.ok(src.includes('e.textContent="No results"'), "模型弹层空态官方同文");
  assert.ok(src.includes('cfCnt.textContent="No results"'), "查找零命中官方同文");
  assert.ok(src.includes('" of "+cfHits.length'), "查找计数 N of M 官方格式");
...
// R217 · 命令审批官方同文。
test("R217: Command Awaiting Approval / Permission required 官方同文", () => {
  const src = fs.readFileSync(path.join(ROOT, "dao-cascade", "panel.js"), "utf8");
  assert.ok(src.includes('header: "Command Awaiting Approval"'), "命令审批浮标官方同文");
...
// R220 · Learn about models 官方同文链接。
test("R220: Learn about models 链接官方同文 + docs 官方 URL", () => {
  const src = fs.readFileSync(path.join(ROOT, "dao-cascade", "panel.js"), "utf8");
...
// R224 · Settings 页 Auto-Run 策略行。
test("R224: Settings 页 Auto-Run 策略行 + 载荷字段", () => {
  const src = fs.readFileSync(path.join(ROOT, "dao-cascade", "settings-page.js"), "utf8");
  assert.ok(src.includes('row("Auto-Run 策略"'), "策略行在位");
