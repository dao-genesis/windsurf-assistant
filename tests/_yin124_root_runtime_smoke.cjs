...
async function main() {
  ...
  console.log(
    "\x1b[90m帛书·四十:「反者道之动·弱者道之用」 · 主公诏:「从根本底层继续验证使用完善」\x1b[0m\n",
  );
  ...
  try {
    ...
    if (h.status === 200 && h.json?.ok === true) {
      ...
      if (
        Array.isArray(h.json.auth?.accepts) &&
        h.json.auth.accepts.length === 4
      )
        ok("/health auth.accepts 4 门");
      else ng("/health auth.accepts", "应 4 门");
      if (h.json.silk?.chars > 0)
        ok(
          "/health silk " + h.json.silk.chars + "字 from " + h.json.silk.source,
        );
      else ng("/health silk", "未载");
    } else ng("/health", "status=" + h.status + " err=" + (h.err || "-"));

    const root = await probe("GET", "/");
    if (root.status === 200 && /真本源单器/.test(root.text))
    ...
    const bad = await probe("GET", "/v1/models", {
    ...
    const m1 = await probe("GET", "/v1/models", {
      Authorization: "Bearer " + AUTH,
    });
    if (
      m1.status === 200 &&
      Array.isArray(m1.json?.data) &&
      m1.json.data.length > 0
    )
    ...
    for (const s of seven) {
      ...
      if (post.status !== 200 || post.json?.ok !== true) {
        ng(`SP POST ${s}`, "status=" + post.status);
        continue;
      }
      const get = await probe("GET", "/v1/system/prompt", {
        "X-Dao-Auth": AUTH,
      });
      if (get.status === 200 && get.json?.strategy === s)
        ok(`SP "${s}" 切 → 验 strategy=${s}`);
      else ng(`SP "${s}" 验`, "期 " + s + " 实 " + get.json?.strategy);
    }
    const badStrat = await probe(
      "POST",
      "/v1/system/prompt",
      { Authorization: "Bearer " + AUTH },
      { strategy: "yin-fake" },
    );
    if (badStrat.status === 400 && Array.isArray(badStrat.json?.allowed))
      ok(`SP 非法态拒 400 allowed=${badStrat.json.allowed.length}`);
    else ng("SP 非法态拒", "应 400 实 " + badStrat.status);

    const setC = await probe(
      "POST",
      "/v1/system/prompt",
      { Authorization: "Bearer " + AUTH },
    ...
    if (setC.status === 200) {
      const getC = await probe("GET", "/v1/system/prompt", {
        Authorization: "Bearer " + AUTH,
      });
      if (getC.json?.customSp?.preview?.includes("反者道之动"))
        ok("SP customSp 真转 · 含「反者道之动」");
    ...
    if (
      models.status === 200 &&
      Array.isArray(models.json?.data) &&
      models.json.data.length >= 10
    )
